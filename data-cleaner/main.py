...
files = st.file_uploader("Upload CSV or Excel Files", type=["csv", "xlse"], accept_multiple_files=True)
...
if files:
    ...
    if st.button(f"⬇️ Download file.name as {format_choice}"):
        ...
        output.seek(0)
        st.download_button(f"⬇️ Download File", file_name=new_name, data=output, mime=mime)
